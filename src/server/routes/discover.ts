...
export async function handleDiscover(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    const step2Results = await Promise.all(
      chosenTop2.map(t2 => {
        ...
        const candidates = stmtGetCategoriesByTop2(db).all(t2);
    ...
    const collapsed = collapseEntries(allEntries);
    const pickupOnly = discFulfillment === 'pickup' && !!discRegionValue;
    const urls = collapsed.map(e => buildTrademeUrl(e, discMaxPrice, discFulfillment, discRegionValue));
