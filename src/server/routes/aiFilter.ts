...
export async function handleAiFilter(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  await Promise.all(batches.map(batch => queue.add(async () => {
    ...
    try {
      const result = await aiJSON(aiCfg, 'ai-filter', AI_FILTER_SYSTEM_MSG, `Criteria: ${prompt}\n\nListings:\n${numbered}`, 4096);
